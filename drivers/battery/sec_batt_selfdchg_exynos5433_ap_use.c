 ...
 *
 * drivers/battery/sec_batt_selfdchg_exynos5433_ap_use.c
 *
 * Drivers for samsung battery self discharging by S/W Policy, with no IC.
 * For only Exynos 5433
 *
...
#ifndef CONFIG_ARM_EXYNOS5433_BUS_DEVFREQ
#error "CONFIG_ARM_EXYNOS5433_BUS_DEVFREQ is not defined, Check policy!"
#endif
/****************************************************************/
...
/* AP Policy Feature */
/******************************************/
#define SDCHG_POLICY_KFC_FREQ_MIN_LIMIT			BIT(0)
#define SDCHG_POLICY_CPU_FREQ_MIN_LIMIT			BIT(1)
...
{
	struct pm_qos_request pm_qos_req_kfc_min;
...
static unsigned int policy_state[SDCHG_STATE_MAX] = {
	...
	/*************************************/
	/* SDCHG_STATE_SET */
	SDCHG_POLICY_KFC_FREQ_MIN_LIMIT |
...
#define SDCHG_MIF_THROUGHPUT 667000
#define SDCHG_INT_THROUGHPUT 334000
...
static void sdchg_policy_set(struct sdchg_info_nochip_t *info)
{
	...
	/*********************************************/
	if (policy_mask & SDCHG_POLICY_KFC_FREQ_MIN_LIMIT) {
		if ( !(policy_mask_prev & SDCHG_POLICY_KFC_FREQ_MIN_LIMIT)) {
			pm_qos_update_request(&pData->pm_qos_req_kfc_min, SDCHG_MIN_FREQ);
		}
	} else {
		if ( policy_mask_prev & SDCHG_POLICY_KFC_FREQ_MIN_LIMIT) {
			pm_qos_update_request(&pData->pm_qos_req_kfc_min, PM_QOS_DEFAULT_VALUE);
		}
	}
	/*********************************************/
...
static void sdchg_exynos5433_ap_use_monitor(void *arg,
...
#error "SDCHG : Implement SOC Read Code!!"
	...
	if (!info->display_on && info->need_state == SDCHG_STATE_SET_LOW) {
		SDCHG_LOG("[SDCHG][%s] little_core1_hotplug_in() ++\n", __func__);
...
static void init_info_data(struct sdchg_info_nochip_t *info)
{
	...
	pm_qos_add_request(&pData->pm_qos_req_kfc_min,
					PM_QOS_KFC_FREQ_MIN,
...
static void remove_info_data(struct sdchg_info_nochip_t *info)
{
	...
	pm_qos_remove_request(&pData->pm_qos_req_kfc_min);
...
/*************************************************************************/
static void sdchg_exynos5433_ap_use_parse_dt(struct device *dev)
{
...
/* prev func : sec_bat_self_discharging_check */
 static void sdchg_exynos5433_ap_use_adc_check(void *arg)
 {
	struct sec_battery_info *battery = (struct sec_battery_info *)arg;
...
static void sdchg_exynos5433_ap_use_ntc_check(void *arg)
{
	struct sec_battery_info *battery = (struct sec_battery_info *)arg;
 ...
 static void sdchg_exynos5433_ap_use_force_control(void *arg, bool dis_en)
 {
	 struct sec_battery_info *battery = (struct sec_battery_info *)arg;
 ...
 static void sdchg_exynos5433_ap_use_discharging_check(void *arg)
 {
	 return;
 }
 
static int sdchg_exynos5433_ap_use_force_check(void *arg)
{
	struct sec_battery_info *battery = (struct sec_battery_info *)arg;
...
static int sdchg_exynos5433_ap_use_probe(void *battery)
{
	int ret = 0;
	struct sdchg_info_nochip_t *info = sdchg_info->nochip;
	pr_info("[SDCHG][%s] ++\n", __func__);
...
static int sdchg_exynos5433_ap_use_remove(void)
{
	struct sdchg_info_nochip_t *info = sdchg_info->nochip;
	pr_info("[SDCHG][%s] ++\n", __func__);
 ...
 static int __init sdchg_exynos5433_ap_use_init(void)
 {
	 int ret;
	...
	sdchg_info->nochip->sdchg_monitor = sdchg_exynos5433_ap_use_monitor;
	/*****************************************/
	sdchg_info->sdchg_probe = sdchg_exynos5433_ap_use_probe;
	sdchg_info->sdchg_remove = sdchg_exynos5433_ap_use_remove;
	sdchg_info->sdchg_parse_dt = sdchg_exynos5433_ap_use_parse_dt;

	sdchg_info->sdchg_adc_check = sdchg_exynos5433_ap_use_adc_check;
#ifndef CONFIG_MACH_UNIVERSAL5433
	sdchg_info->sdchg_ntc_check = sdchg_exynos5433_ap_use_ntc_check;
#endif
	sdchg_info->sdchg_force_control = sdchg_exynos5433_ap_use_force_control;
	sdchg_info->sdchg_discharging_check = sdchg_exynos5433_ap_use_discharging_check;

	sdchg_info->sdchg_force_check = sdchg_exynos5433_ap_use_force_check;
	/*****************************************/
...
static void __exit sdchg_exynos5433_ap_use_exit(void)
{
	pr_info("[SDCHG][%s] ++\n", __func__);
...
arch_initcall(sdchg_exynos5433_ap_use_init);
module_exit(sdchg_exynos5433_ap_use_exit);
