 ...
 *
 * drivers/battery/sec_batt_selfdchg_exynos3475_ap_use.c
 *
 * Drivers for samsung battery self discharging by S/W Policy, with no IC.
 * For only Exynos 3475
 *
...
#error "CONFIG_HOTPLUG_CPU is not defined, Check policy!"
#endif

/* Exynos Feature */
#ifndef CONFIG_ARM_EXYNOS3475_BUS_DEVFREQ
#error "CONFIG_ARM_EXYNOS3475_BUS_DEVFREQ is not defined, Check policy!"
#endif
/****************************************************************/
...
#define SDCHG_POLICY_CPU_FREQ_MIN_LIMIT			BIT(0)
#define SDCHG_POLICY_CPU_HOTPLUG_DISABLE			BIT(1)
...
{
	struct pm_qos_request pm_qos_req_cpu_min;
...
static unsigned int policy_state[SDCHG_STATE_MAX] = {
	...
	/*************************************/
	/* SDCHG_STATE_SET */
	SDCHG_POLICY_CPU_FREQ_MIN_LIMIT |
	SDCHG_POLICY_CPU_HOTPLUG_DISABLE |
...
/*******************************************************/
/* for cpu hotplug disable  */
/*******************************************************/
#define SDCHG_BATT_CHECK_DELAY_HOTPLUG		5

static int cpu_hotplug_disabled_for_sdchg;
unsigned int sdchg_cpu_hotplug_mask;
bool sdchg_cpu_hotplug_disable_set;

void sdchg_cpu_hotplug_enable(void)
{
	cpu_maps_update_begin();
	cpu_hotplug_disabled_for_sdchg = 0;
	cpu_maps_update_done();
}

int sdchg_cpu_hotplug_disable(unsigned int cpu_mask)
{
	int err = 0;
	int cpu = 0;

	for (cpu = 1; cpu < NR_CPUS; cpu++) {
		if (!((cpu_mask >> cpu) & 0x01))
			continue;

		/*
		if (cpu_online(cpu)) {
			SDCHG_LOG("[GPIO_IR][%s] cpu %d already online\n", __func__, cpu);
			continue;
		}
		*/
		...
		err = cpu_up(cpu);
		if (err) {
			pr_err("[SDCHG][%s] Error (%d) online core %d\n", 
				__func__, err, cpu);
			goto out;
		}
	}
	...
	for (cpu = 1; cpu < NR_CPUS; cpu++) {
		...
		if (!cpu_online(cpu)) {
			err = -EBUSY;
			pr_info("[SDCHG][%s] Error, cpu_down(%d) is called to another code!\n",
				__func__, cpu);
			cpu_maps_update_done();
			goto out;
		}
	}
	cpu_hotplug_disabled_for_sdchg = 1;
	cpu_maps_update_done();
...
static int sdchg_cpu_hotplug_set(struct sdchg_info_nochip_t *info)
{
	int ret = 0;

	if ( policy_state[info->set_state] & SDCHG_POLICY_CPU_HOTPLUG_DISABLE) {
		if (!sdchg_cpu_hotplug_disable_set) {
			ret = sdchg_cpu_hotplug_disable(sdchg_cpu_hotplug_mask);
			if (!ret) {
				sdchg_cpu_hotplug_disable_set = true;
			}
		}
	} else {
		if (sdchg_cpu_hotplug_disable_set) {
			sdchg_cpu_hotplug_enable();
			sdchg_cpu_hotplug_disable_set = false;
		}
	}
...
static int __cpuinit sdchg_cpu_callback(struct notifier_block *nfb,
					unsigned long action, void *hcpu)
{
	unsigned int cpu = (unsigned long)hcpu;

	// As "cpu_maps_update_begin" has been run already,
	// it doesn't need to be run again.
	if (cpu_hotplug_disabled_for_sdchg) {
		if ((action == CPU_DOWN_PREPARE) &&
			(sdchg_cpu_hotplug_mask & (1 << cpu))) {
			return notifier_from_errno(-EBUSY);	// this causes "cpu_down" to fail
		}
	}
...
#define SDCHG_MIF_THROUGHPUT 413000
#define SDCHG_INT_THROUGHPUT 275000
...
static void sdchg_policy_set(struct sdchg_info_nochip_t *info)
{
	...
	/*********************************************/
	if (policy_mask & SDCHG_POLICY_CPU_FREQ_MIN_LIMIT) {
		if ( !(policy_mask_prev & SDCHG_POLICY_CPU_FREQ_MIN_LIMIT)) {
			pm_qos_update_request(&pData->pm_qos_req_cpu_min, SDCHG_MIN_FREQ);
		}
	} else {
		if ( policy_mask_prev & SDCHG_POLICY_CPU_FREQ_MIN_LIMIT) {
			pm_qos_update_request(&pData->pm_qos_req_cpu_min, PM_QOS_DEFAULT_VALUE);
		}
	}
	/*********************************************/
	if (policy_mask & SDCHG_POLICY_CPU_IDLE_POLL_CTRL) {
		if (!(policy_mask_prev & SDCHG_POLICY_CPU_IDLE_POLL_CTRL)) {
			sdchg_idle_policy_set = true;
		}
	} else {
		if (policy_mask_prev & SDCHG_POLICY_CPU_IDLE_POLL_CTRL) {
			sdchg_idle_policy_set = false; 
		}
	}
	/*********************************************/
...
extern bool sdchg_charger_attached(struct sec_battery_info *battery);

static void sdchg_exynos3475_ap_use_monitor(void *arg,
	...
	if (info->set_state != info->need_state) {
		...
		if (info->set_state == SDCHG_STATE_NONE) {	// none -> discharing
			...
			if (info->need_state == SDCHG_STATE_NONE) {	// discharging -> none
				if (sdchg_charger_attached(battery) || battcond >= SDCHG_BATTCOND_START) {
					need_set_alarm = info->state_machine_run = true;
...
//KEEP_GOING:
	...
	if (sdchg_cpu_hotplug_set(info)) {
		need_set_alarm = true;
		set_alarm_time = SDCHG_BATT_CHECK_DELAY_HOTPLUG;
	}
...
static void init_info_data(struct sdchg_info_nochip_t *info)
{
	...
	pm_qos_add_request(&pData->pm_qos_req_cpu_min,
					PM_QOS_CPU_FREQ_MIN,
...
static void remove_info_data(struct sdchg_info_nochip_t *info)
{
	...
	pm_qos_remove_request(&pData->pm_qos_req_cpu_min);
...
/*************************************************************************/
static void sdchg_exynos3475_ap_use_parse_dt(struct device *dev)
{
...
/* prev func : sec_bat_self_discharging_check */
 static void sdchg_exynos3475_ap_use_adc_check(void *arg)
 {
	struct sec_battery_info *battery = (struct sec_battery_info *)arg;
...
static void sdchg_exynos3475_ap_use_ntc_check(void *arg)
{
	struct sec_battery_info *battery = (struct sec_battery_info *)arg;
 ...
 static void sdchg_exynos3475_ap_use_force_control(void *arg, bool dis_en)
 {
	 struct sec_battery_info *battery = (struct sec_battery_info *)arg;
 ...
 static void sdchg_exynos3475_ap_use_discharging_check(void *arg)
 {
	 return;
 }
 
static int sdchg_exynos3475_ap_use_force_check(void *arg)
{
	struct sec_battery_info *battery = (struct sec_battery_info *)arg;
...
static int sdchg_exynos3475_ap_use_probe(void *battery)
{
	int ret = 0;
	struct sdchg_info_nochip_t *info = sdchg_info->nochip;
	pr_info("[SDCHG][%s] ++\n", __func__);
	...
	sdchg_cpu_hotplug_mask = 0x0F;
	sdchg_idle_poll_mask = 0x0F;

	hotcpu_notifier(sdchg_cpu_callback, CPU_PRI_SCHED_INACTIVE);
...
static int sdchg_exynos3475_ap_use_remove(void)
{
	struct sdchg_info_nochip_t *info = sdchg_info->nochip;
	pr_info("[SDCHG][%s] ++\n", __func__);
 ...
 static int __init sdchg_exynos3475_ap_use_init(void)
 {
	 int ret;
	...
	sdchg_info->nochip->sdchg_monitor = sdchg_exynos3475_ap_use_monitor;
	/*****************************************/
	sdchg_info->sdchg_probe = sdchg_exynos3475_ap_use_probe;
	sdchg_info->sdchg_remove = sdchg_exynos3475_ap_use_remove;
	sdchg_info->sdchg_parse_dt = sdchg_exynos3475_ap_use_parse_dt;

	sdchg_info->sdchg_adc_check = sdchg_exynos3475_ap_use_adc_check;
	sdchg_info->sdchg_ntc_check = sdchg_exynos3475_ap_use_ntc_check;
	sdchg_info->sdchg_force_control = sdchg_exynos3475_ap_use_force_control;
	sdchg_info->sdchg_discharging_check = sdchg_exynos3475_ap_use_discharging_check;

	sdchg_info->sdchg_force_check = sdchg_exynos3475_ap_use_force_check;
	/*****************************************/
...
static void __exit sdchg_exynos3475_ap_use_exit(void)
{
	pr_info("[SDCHG][%s] ++\n", __func__);
...
arch_initcall(sdchg_exynos3475_ap_use_init);
module_exit(sdchg_exynos3475_ap_use_exit);
